...
describe OpenWeatherMaps do 
  ...
  context 'Testing of weather data' do 
    
    before(:all) do
      @api_key = ENV['API_KEY']
      @weather_data = OpenWeatherMaps.new.weather
      @weather_data.retrieve('524901', @api_key)
    end

    it 'should return coord to be a hash' do
      expect(@weather_data.get_coord).to be_kind_of(Hash)
    end

    it 'should return lon to be a float' do
      expect(@weather_data.get_coord_lon).to be_kind_of(Float)
    end

    it 'should return lat to be a float' do
      expect(@weather_data.get_coord_lat).to be_kind_of(Float)
    end

    it 'should return weather to be an array' do
      expect(@weather_data.get_weather).to be_kind_of(Array)
    end
    ...
    it 'should return speed to be an integer' do
      expect(@weather_data.get_wind_speed).to be_kind_of(Integer)
    end
    ...
    it 'should return all to be an integer' do
      expect(@weather_data.get_clouds_all).to be_kind_of(Integer)
    end
